...
class locationQueryAction extends EmtAction
{
    public function execute($request)
    {
        ...
        $cc = strtoupper($this->getRequestParameter('cc'));
        $lc = $this->getRequestParameter('lc')==1;
        $nb = $this->getRequestParameter('nb')==1;
        
        if (!preg_match("/^[A-Z]{2}$/", $cc)) return $this->renderText($this->getRequestParameter('callback') != '' ? $this->getRequestParameter('callback')."(".json_encode(array()).");" : json_encode(array()));
        ...
        $res = json_encode(isset($neighbours) ? array('neighbours' => $neighbours, 'cities' => $cities)  : $cities );
