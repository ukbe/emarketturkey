...
<div>
<div class="items">
<?php echo link_to(__('Ignore'), 'mycareer/print', 'class=act a16px hand' . ($app->getFlagType() == UserJobPeer::UJ_EMPLYR_FLAG_IGNORE ? ' flagged' : '')) ?>
<?php echo link_to(__('Favourite'), 'mycareer/print', 'class=act a16px star' . ($app->getFlagType() == UserJobPeer::UJ_EMPLYR_FLAG_FAVOURITE ? ' flagged' : '')) ?>
<?php echo link_to(__('None'), 'mycareer/print', 'class=act a16px' . (!$app->getFlagType() ? ' flagged' : '')) ?>
</div>
...
<table class="cvcard">
<tr>
    <td class="col1"><?php echo image_tag($resume->getPhotoUri(MediaItemPeer::LOGO_TYPE_MEDIUM)) ?></td>
    ...
    <td class="col3">
        <div class="_dotted">
            <div class="_right t_orange"><?php echo __('Evaluating') ?></div>
            <?php echo __('Current Status:') ?>
			<div class="clear"></div>
            <div class="stat-chg _right"><?php echo link_to_function(__('change'), "$('.stat-chg').toggle();", 'class=bluelink') ?></div>
			<div class="stat-chg-box stat-chg ghost">
				<?php echo __("Please select a new status for %1's application:", array('%1' => $app->getUser())) ?>
                <div class="hrsplit-1"></div>
				<?php echo select_tag('status', options_for_select(array(1 => __('Evaluating'), 2 => __('Accepted'), 3 => __('Rejected'), 4 => __('Employed')), null, array('include_custom' => __('Please Select')))) ?>
                <div class="hrsplit-1"></div>
                <div class="_right"><?php echo link_to_function(__('cancel'), "$('.stat-chg').toggle();$('.folder-chg').toggle();", 'class=bluelink') ?></div>
        ...
        <div class="_dotted">
			...
			<div class="hrsplit-1"></div>
            <span class="tag trail-right"><?php echo $app->getFolderId() ? $app->getResumeFolder() : __('Not Classified') ?></span>
            <div class="folder-chg _right"><?php echo link_to_function(__('change'), "$('.folder-chg').toggle();$('.stat-chg').toggle();", 'class=bluelink') ?></div>
			<div class="folder-chg-box folder-chg ghost">
				<?php echo __("Please select a folder to store %1's resume:", array('%1' => $app->getUser())) ?>
                <div class="hrsplit-1"></div>
				<?php echo select_tag('folder_id', options_for_select($profile->getResumeFolders(), null, array('include_custom' => __('Please Select')))) ?>
                <div class="hrsplit-1"></div>
                <div class="_right"><?php echo link_to_function(__('cancel'), "$('.stat-chg').toggle();", 'class=bluelink') ?></div>
                <?php echo submit_tag(__('Save Status'), 'class=green-button smaller') ?>
			</div>
            <div class="hrsplit-1"></div>
        </div>
