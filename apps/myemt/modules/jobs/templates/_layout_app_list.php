...
<?php foreach ($results as $app): ?>
    <tr<?php echo !$app->getEmployerRead() ? ' class="unread"' : '' ?>>
        <td><?php echo link_to(image_tag($app->getUser()->getResume() ? $app->getUser()->getResume()->getPhotoUri() : $app->getUser()->getProfilePictureUri()), $url = $job->getApplicantUrl($app->getId())) ?></td>
