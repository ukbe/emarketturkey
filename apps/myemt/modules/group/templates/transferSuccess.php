    ...
    <div class="col_576">
        <div class="box_576 _titleBG_Transparent">
            ...
            <dl class="_table">
                ...
                <dt></dt>
                <dd><?php echo link_to(__('Cancel Transfer'), "@group-account?action=transfer&act=cancel&tid={$transfer->getGuid()}&hash={$group->getHash()}", 'class=green-button') ?></dd>
                <?php endif ?>
            </dl>
            <div class="hrsplit-1"></div>
            <?php echo link_to(__('Re-send notification email to %1.', array('%1' => $transfer->getUserRelatedByNewOwnerId())), "@group-account?action=transfer&act=notify&hash={$group->getHash()}", 'class=act a16px message bluelink') ?>
            ...
            <?php endif ?>
            <?php if (!$member): ?>
            <?php echo __('Type email address* of the member who you want to transfer account ownership of the group <b>%1</b>.', array('%1' => $group)) ?>
            <em class="ln-example">* <?php echo __('Make sure to enter the registered email address of member.') ?></em>
            <div class="hrsplit-2"></div>
            <?php echo form_errors() ?>
            <?php echo form_tag("@group-account?action=transfer&hash={$group->getHash()}") ?>
            <dl class="_table">
            ...
            <?php echo link_to(__('No! Go Back'), "@group-account?action=transfer&hash={$group->getHash()}", 'class=inherit-font bluelink hover') ?>
            </form>
            </div>
            <?php endif ?>
            <?php endif ?>
