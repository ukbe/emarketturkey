...
<li class="last"><?php echo substr($company, 0, 57).(strlen($company)>57?' ...':'') ?></li>
</ol>
<?php end_slot() ?>
<?php slot('leftcolumn') ?>
...
<?php $company_users = $company->getCompanyUsers() ?>
<table class="monocolor" width="95%" cellpadding="4" cellspacing="0">
<?php foreach ($company_users as $company_user): ?>
<tr><th><b><?php echo __('User') ?></b></th><td>
<?php echo link_to(($usr = $company_user->getObject()), 'admin/user', array('query_string' => 'id='.$usr->getId()))?> - <?php echo $company_user->getRole()->getName() ?></td></tr>
<?php endforeach ?>
</table>
<div class="hrsplit-2"></div>
...
<tr><td><b><?php echo $event->getActionCase()->getAction()->getName() ?></b></td><td><?php echo format_datetime($event->getCreatedAt('U')) ?></td></tr>
<?php endforeach ?>
<?php else: ?>
