...
<?php include_partial('mycareer/leftmenu', array('sesuser' => $sesuser))?>
    ...
    <div class="col_576">
        ...
        <div class="box_576">
            <h4><?php echo __('References') ?></h3>
            <div class="_noBorder pad-0">
                <?php  if ($message = $sf_user->getFlash('message')): ?>
                <div class="bubble pad-2"><?php echo __($message)?></div>
                <?php endif ?>
                <?php foreach($references as $sch): ?>
                <?php if ($reference && $sch->getId() == $reference->getId()): ?>
                <?php echo $objhandler->render($reference, $act) ?>
                <?php else: ?>
                ...
                <?php if ($act == 'new'): ?>
                <?php echo $objhandler->render($reference, 'new') ?>
                <?php else:?>
                <div class="pad-1 margin-b2">
                <?php echo link_to(__('Add Reference'), "@mycv-action?action=references&act=new", 'id=addnew class=ln-addlink greenspan led add-11px ajax-enabled') ?>
                </div>
                <?php endif ?>

                <?php echo form_tag('@mycv-action?action=references') ?>
                <?php echo input_hidden_tag('done', __('Done')) ?>
                <?php echo input_hidden_tag('next', __('Next')) ?>
                <div class="txtCenter">
                    <?php echo link_to(__('Back'), '@mycv-action?action=awards', 'class=action-button _left')?>
