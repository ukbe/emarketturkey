...
<?php include_partial('mycareer/leftmenu', array('sesuser' => $sesuser))?>
    ...
    <div class="col_576">
        ...
        <div class="box_576">
            <h4><?php echo __('Publications') ?></h3>
            <div class="_noBorder pad-0">
                <?php  if ($message = $sf_user->getFlash('message')): ?>
                <div class="bubble pad-2"><?php echo __($message)?></div>
                <?php endif ?>
                <?php foreach($publications as $sch): ?>
                <?php if ($publication && $sch->getId() == $publication->getId()): ?>
                <?php echo $objhandler->render($publication, $act) ?>
                <?php else: ?>
                ...
                <?php if ($act == 'new'): ?>
                <?php echo $objhandler->render($publication, 'new') ?>
                <?php else:?>
                <div class="pad-1 margin-b2">
                <?php echo link_to(__('Add Publication'), "@mycv-action?action=publications&act=new", 'id=addnew class=ln-addlink greenspan led add-11px ajax-enabled') ?>
                </div>
                <?php endif ?>

                <?php echo form_tag('@mycv-action?action=publications') ?>
                <?php echo input_hidden_tag('done', __('Done')) ?>
                <?php echo input_hidden_tag('next', __('Next')) ?>
                <div class="txtCenter">
                    <?php echo link_to(__('Back'), '@mycv-action?action=languages', 'class=action-button _left')?>
