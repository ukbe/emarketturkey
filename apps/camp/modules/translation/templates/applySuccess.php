...
<div class="col_948">
    ...
    <div class="col_657 clear _center">
        <div class="box_657">
            ...
            <?php else: ?>
            <?php echo __('Please select the account you want to use while applying for Translator:') ?>
            <ul class="select-step">
            <?php foreach ($accounts as $acc):?>
            <?php $tx = TranslatorPeer::retrieveAccountFor(array(TranslatorPeer::TR_STAT_APPROVED, TranslatorPeer::TR_STAT_PENDING, TranslatorPeer::TR_STAT_SUSPENDED), $acc) ?>
            <li<?php echo $tx ? ' class="disabled"' : ''?>>
                ...
                <?php if ($tx): ?>
                    ...
                    <p><?php echo __('There is an existing Translator account associated with this account.') ?><br />
                    <?php if ($tx->getStatus() == TranslatorPeer::TR_STAT_APPROVED): ?>
                    <?php echo __('Follow %1 to configure this Translator account.', array('%1' => link_to(__('<span class="followthis">this link</span>'), "@manage-translator?hash={$acc->getPlug()}", 'class=inherit-font bluelink hover'))) ?>
                    <?php endif ?>
                    </p>
                <?php else:?>
                    ...
                    <p><?php echo __('Select this if you would like to apply for an individual Translator account.') ?><br />
                    <em class="t_grey"><?php echo __('This way you will be listed as Translator(Professional).') ?></em></p>
                    <?php else: ?>
                    <p><?php echo __('Select this if you would like to apply for a corporate Translator account.') ?><br />
                    <em class="t_grey"><?php echo __('If accepted, your company will be listed as Translator(Corporation).') ?></em></p>
                    <?php endif ?>
                <?php endif ?>
                </li>
            <?php endforeach ?>
