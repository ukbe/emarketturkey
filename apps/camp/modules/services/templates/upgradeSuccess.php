        ...
        <div class="box_678 noBorder banner" style="">
                ...
                <div>
                    ...
                    <div id='crmWebToEntityForm' align=center>
                        ...
                        <?php if ($campaign): ?>
                            <?php echo input_hidden_tag('LEADCF6', $campaign->getCode()) ?>
                        ...
                        <?php endif ?>
                            ...
                            <script> var mndFileds=new Array(<?php if ($campaign && $campaign->getCode() == 'tuskon.spr2013') echo "'LEADCF5', "?>'Company', 'First Name', 'Last Name', 'Email', 'Phone');var fldLangVal=new Array(<?php if ($campaign && $campaign->getCode() == 'tuskon.spr2013') echo '"'.__('Related Association').'", ' ?>"<?php echo __('Company Name') ?>", "<?php echo __('Name') ?>", "<?php echo __('Lastname') ?>", "<?php echo __('E-mail') ?>", "<?php echo __('Phone') ?>");function checkMandatery(){for(i=0;i<mndFileds.length;i++){ var fieldObj=document.forms['WebToLeads695090000000053037'][mndFileds[i]];if(fieldObj) {if(((fieldObj.value).replace(/^\s+|\s+$/g, '')).length==0){alert(("<?php echo __('%1field cannot be empty') ?>").replace('%1field', fldLangVal[i])); fieldObj.focus(); return false;}else if(fieldObj.nodeName=='SELECT'){if(fieldObj.options[fieldObj.selectedIndex].value == 'null'){alert(("<?php echo __('Please select %1field from list') ?>").replace('%1field', fldLangVal[i])); fieldObj.focus(); return false;}}}}}</script>  
                        </form>
                    </div>                
                </div>
            </div>
