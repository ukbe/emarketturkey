...
<div id="topbar">
    <div class="column span-44 prepend-1">
        <?php echo link_to(image_tag('emtlogo.gif', array('alt' => 'Homepage', 'width' => 210)), $sf_user->getUser() ? '@myemt.homepage' : '@lobby.homepage') ?></div>
    ...
    <?php if ($sf_user->getUser() && !$sf_user->getUser()->getLogin()->isVerified()): ?>
        <div style="margin-top: 40px;"><?php echo link_to(__('Verification Required'), '@myemt.verify-email', 'class=lock-16px') ?></div>
    <?php endif ?>
                </div>
    </div>
</div>