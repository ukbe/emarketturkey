...
<td>
    <?php echo link_to("<sup>".($notcnt = $sf_user->getUser()->getNotificationCount())."</sup>", '@notifications', array('id' => 'notifications', 'class' => 'toplink'. ($notcnt ? ' new': ''), 'title' => __('Notifications'))) ?>
</td>
<td>
    <?php echo link_to("<sup>".($reqcnt = $sf_user->getUser()->getRequestCount())."</sup>", $reqcnt ? '@requests' : '@network', array('class' => 'toplink contacts'. ($reqcnt ? ' new': ''), 'title' => __('Contacts'))) ?>
</td>
