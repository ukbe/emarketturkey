...
<?php echo link_to(image_tag($user->getProfilePictureUri(true), 'width=200'), 'media/photos?id='.$user->getId()) ?>
<ol class="profile-actions">
<li><?php echo link_to(__('See Photos'), 'media/photos?id='.$user->getId()) ?></li>
<?php if ($sf_user->getUser()->can(ActionPeer::ACT_SEND_MESSAGE, $user)): ?>
<li><?php echo link_to(__('Send Message'), 'messages/compose', array('query_string' => 'rcpu='.$user->getId().'&_ref='.$sf_request->getUri())) ?></li>
<?php endif ?>
<?php if ($sesuser->getId()!=$user->getId() && !$sesuser->isFriendsWith($user->getId()) && $sesuser->can(ActionPeer::ACT_ADD_TO_NETWORK, $user)): ?>
<li><?php echo link_to(__('Add as Friend'), 'profile/add', array('query_string' => "token=$token&id=".$user->getId().'&width=560&height=200', 'class' => 'thickbox', 'title' => __('Add as Friend'))) ?></li>
<?php endif ?>
</ol>
...
<?php if ($sf_user->getUser()->can(ActionPeer::ACT_VIEW_PROFILE, $friend)): ?>
