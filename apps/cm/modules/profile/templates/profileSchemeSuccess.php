<?php use_helper('Date') ?>
<?php slot('uppermenu') ?>
<?php include_partial('profile/profiletop', array('user' => $user)) ?>
<?php end_slot() ?>
<?php if ($pvars['profile']): ?>
<?php slot('leftcolumn') ?>
...
<li><?php echo link_to(__('Add as Friend'), "@user-action?action=add&id={$user->getId()}", array('query_string' => "token=$token&width=560&height=220&_ref=".urlencode($sf_request->getUri()), 'class' => 'thickbox add', 'title' => __('Add as Friend'))) ?></li>
<?php elseif ($sesuser->isFriendsWith($user->getId())): ?>
<li><?php echo link_to(__('Edit Relationship'), "@user-action?action=add&id={$user->getId()}", array('query_string' => "token=$token&width=510&height=130&mod=chanrel", 'class' => 'thickbox', 'title' => __('Edit Relationship'))) ?></li>
<li><?php echo link_to(__('Remove from Friends'), "@user-action?action=add&id={$user->getId()}", array('query_string' => "token=$token&width=510&height=130&mod=remove", 'class' => 'thickbox', 'title' => __('Remove from Friends'))) ?></li>
<?php endif ?>
<li class="left-box">
<span class="header"><?php echo __('Friends').'('.count($friends).')' ?>
<?php echo link_to(__('see all'), $user->getProfileActionUrl('friends'), 'class=action') ?></span>
<?php $fc = 0; ?>
...
<?php echo link_to(__('see all'), $user->getProfileActionUrl('groups'), 'class=action') ?></span>
<ol>
<?php $gr = 0; ?>
...
<?php elseif (!$sf_user->getUser() || $sesuser->getId()!=$user->getId()): ?>
...
<div class="hrsplit-2"></div>
 <ol class="profile-tabs">
 <?php foreach ($tabs as $key => $tab): ?>
     <li><?php echo link_to('<span>'.__($tab[0]).'</span>', $tab[1], $key==$tabindex ? 'class=selected': '') ?></li>
 <?php endforeach ?>
 </ol>
<div class="pad-2">
<?php include_partial($sf_context->getActionName(), $pvars) ?>
</div>