...
<?php endif ?>
<li><?php echo link_to(count($userprops)>1 ? __('Edit Membership') : __('Join Group'), "@group-action?stripped_name={$sf_params->get('stripped_name')}&action=join&token=".sha1(base64_encode($group.session_id()))."&cid={$group->getId()}&width=560&height=200", array('class' => 'thickbox', 'title' => count($userprops)>1 ? __('Edit Membership') : __('Join Group'))) ?></li>
<li class="left-box">
<span class="header"><?php echo __('People').'(<span id="group-people-count">'.count($people).'</span>)' ?>
<?php echo link_to_function(__('see all'), $group->getProfileActionUrl('people'), 'class=action') ?></span>
<?php $mc = 0; ?>
...
<?php echo link_to_function(__('see all'), $group->getProfileActionUrl('companies'), 'class=action') ?></span>
<?php $mc = 0; ?>
