...
<?php echo ($sf_user->getUser() && ($sf_user->getUser()->isOwnerOf($comment->getItemId(), $comment->getItemTypeId()) ||
                                    $sf_user->getUser()->isOwnerOf($comment->getId(), PrivacyNodeTypePeer::PR_NTYP_COMMENT))) ? link_to_remote(__('x'), array('url' => '@cm.profile-action?action=comment&mod=rm&hash='.$comment->getHash(), 'update' => 'cm-'.$comment->getHash()), 'class=command') : '' ?>
</div>
</div>
<?php endforeach ?>
<?php endif ?>
...
<?php echo __('%1L or %2S to leave comment.', array('%1L' => link_to(__('Login'), '@myemt.login', array('query_string' => '_ref='.urlencode(sfContext::getInstance()->getRequest()->getUri()), 'absolute' => true)), '%2S' => link_to(__('Sign Up'), '@lobby.signup', array('query_string' => '_ref='.urlencode(sfContext::getInstance()->getRequest()->getUri()), 'absolute' => true)))) ?>
<?php endif ?>
</div>