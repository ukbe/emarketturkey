...
<li id="f<?php echo $cname.'all' ?>"<?php echo ($sf_params->get($data['criteria'])==''?' class="selected"':'') ?>><?php echo link_to(__('All') . ' ('. $data['total'] .')', $searchroute, array('query_string' => http_build_query($criteria))) ?></li>
<?php foreach ($data['results'] as $datum): ?>
<?php $criteria[$data['criteria']] = $datum['object']->getId() ?>
<li id="f<?php echo $cname.$i ?>"<?php echo ($sf_params->get($data['criteria'])==$datum['object']->getId()?' class="selected"':'') ?>><?php echo link_to($datum['object'] . ' ('.$datum['count'].')', $searchroute, array('query_string' => http_build_query($criteria))) ?></li>
<?php $i++ ?>
<?php endforeach ?>
<?php endforeach ?>
</ol>
