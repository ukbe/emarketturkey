...
class diff_patch {
    ...
    public static function diff($a, $b, $min=3, $i=0)
    {
        $diff = array();
        if($a == "" && $b == "") return $diff;
        if($a == "")
        ...
        $ap = strpos($a, $match);
        $bp = strpos($b, $match);
        $diff = self::diff(substr($a, 0, $ap), substr($b, 0, $bp), $min, $i);
        return array_merge($diff, self::diff(substr($a, $ap+strlen($match)), substr($b, $bp+strlen($match)), $min, $i+$bp+strlen($match)));
    }
    
    /**
    * Find the longest match between two strings.
    * The time limit must be turned off for this public static function.
    ...
    */
    public static function patch($text, $diff)
    {
        if(!is_array($diff))
        {
            $n = 0;
