    ...
    foreach ($this->getTables() as $table => $columns)
    {
      ...
      foreach ($columns as $column => $attributes)
      {
        if ($attributes == null)
        {
          ...
          if (strtolower($column) == 'created_at' || strtolower($column) == 'updated_at')
          {
  ...
  protected function fixXMLColumns()
  {
    foreach ($this->getTables() as $table => $columns)
    {
      foreach ($columns as $column => $attributes)
      {
        ...
        if ((strtolower($column) == 'created_at') || (strtolower($column) == 'updated_at') && !array_diff($attributes, array('type' => 'timestamp')))
        {
          // simplify timestamps
