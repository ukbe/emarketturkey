...
/**
 * Base class that represents a row from the 'EMT_EVENT_I18N' table.
 *
 * 
 *
...
abstract class BaseEventI18n extends BaseObject  implements Persistent {
  ...
  const PEER = 'EventI18nPeer';
	...
	public function setIntroduction($v)
	{
		...
		if ($this->introduction !== $v) {
			$this->introduction = $v;
			$this->modifiedColumns[] = EventI18nPeer::INTRODUCTION;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 4; // 4 = EventI18nPeer::NUM_COLUMNS - EventI18nPeer::NUM_LAZY_LOAD_COLUMNS).
    ...
    foreach (sfMixer::getCallables('BaseEventI18n:delete:pre') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseEventI18n:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseEventI18n:save:pre') as $callable)
    {
      ...
      if (is_int($affectedRows))
      {
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BaseEventI18n:save:post') as $callable)
    {
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(EventI18nPeer::INTRODUCTION)) $criteria->add(EventI18nPeer::INTRODUCTION, $this->introduction);
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseEventI18n:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseEventI18n::%s', $method));
    }
