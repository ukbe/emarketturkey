...
abstract class BasePrivacyNodeType extends BaseObject  implements Persistent {
  ...
  const PEER = 'PrivacyNodeTypePeer';
	 ...
	 */
	protected $collWallIgnoresRelatedByOwnerTypeId;

	/**
	 * @var        Criteria The criteria used to select the current contents of collWallIgnoresRelatedByOwnerTypeId.
	 */
	private $lastWallIgnoreRelatedByOwnerTypeIdCriteria = null;

	/**
	 * @var        array WallIgnore[] Collection to store aggregation of WallIgnore objects.
	 */
	protected $collWallIgnoresRelatedByPosterTypeId;

	/**
	 * @var        Criteria The criteria used to select the current contents of collWallIgnoresRelatedByPosterTypeId.
	 */
	private $lastWallIgnoreRelatedByPosterTypeIdCriteria = null;

	/**
	 * @var        array WallIgnore[] Collection to store aggregation of WallIgnore objects.
	 */
	protected $collWallIgnoresRelatedByItemTypeId;

	/**
	 * @var        Criteria The criteria used to select the current contents of collWallIgnoresRelatedByItemTypeId.
	 */
	private $lastWallIgnoreRelatedByItemTypeIdCriteria = null;
	...
	/**
	 * @var        array PremiumAccount[] Collection to store aggregation of PremiumAccount objects.
	 */
	protected $collPremiumAccounts;

	/**
	 * @var        Criteria The criteria used to select the current contents of collPremiumAccounts.
	 */
	private $lastPremiumAccountCriteria = null;
  ...
  protected $culture;
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		if ($deep) {  // also de-associate any related objects?
			...
			$this->collWallIgnoresRelatedByOwnerTypeId = null;
			$this->lastWallIgnoreRelatedByOwnerTypeIdCriteria = null;

			$this->collWallIgnoresRelatedByPosterTypeId = null;
			$this->lastWallIgnoreRelatedByPosterTypeIdCriteria = null;

			$this->collWallIgnoresRelatedByItemTypeId = null;
			$this->lastWallIgnoreRelatedByItemTypeIdCriteria = null;
			...
			$this->collPremiumAccounts = null;
			$this->lastPremiumAccountCriteria = null;
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->collWallIgnoresRelatedByOwnerTypeId !== null) {
				foreach ($this->collWallIgnoresRelatedByOwnerTypeId as $referrerFK) {
					if (!$referrerFK->isDeleted()) {
						$affectedRows += $referrerFK->save($con);
					}
				}
			...
			if ($this->collWallIgnoresRelatedByPosterTypeId !== null) {
				foreach ($this->collWallIgnoresRelatedByPosterTypeId as $referrerFK) {
					if (!$referrerFK->isDeleted()) {
						$affectedRows += $referrerFK->save($con);
					}
				}
			...
			if ($this->collWallIgnoresRelatedByItemTypeId !== null) {
				foreach ($this->collWallIgnoresRelatedByItemTypeId as $referrerFK) {
					if (!$referrerFK->isDeleted()) {
						$affectedRows += $referrerFK->save($con);
					}
				}
			...
			if ($this->collPremiumAccounts !== null) {
				foreach ($this->collPremiumAccounts as $referrerFK) {
					if (!$referrerFK->isDeleted()) {
						$affectedRows += $referrerFK->save($con);
					}
				}
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		if ($deepCopy) {
			...
			foreach ($this->getWallIgnoresRelatedByOwnerTypeId() as $relObj) {
				if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
					$copyObj->addWallIgnoreRelatedByOwnerTypeId($relObj->copy($deepCopy));
				}
			}

			foreach ($this->getWallIgnoresRelatedByPosterTypeId() as $relObj) {
				if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
					$copyObj->addWallIgnoreRelatedByPosterTypeId($relObj->copy($deepCopy));
				}
			}

			foreach ($this->getWallIgnoresRelatedByItemTypeId() as $relObj) {
				if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
					$copyObj->addWallIgnoreRelatedByItemTypeId($relObj->copy($deepCopy));
				}
			}
			...
			foreach ($this->getPremiumAccounts() as $relObj) {
				if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
					$copyObj->addPremiumAccount($relObj->copy($deepCopy));
				}
			}
	...
	/**
	 * Clears out the collWallIgnoresRelatedByOwnerTypeId collection (array).
	 *
	 * This does not modify the database; however, it will remove any associated objects, causing
	 * them to be refetched by subsequent calls to accessor method.
	 *
	 * @return     void
	 * @see        addWallIgnoresRelatedByOwnerTypeId()
	 */
	public function clearWallIgnoresRelatedByOwnerTypeId()
	{
		$this->collWallIgnoresRelatedByOwnerTypeId = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collWallIgnoresRelatedByOwnerTypeId collection (array).
	 *
	 * By default this just sets the collWallIgnoresRelatedByOwnerTypeId collection to an empty array (like clearcollWallIgnoresRelatedByOwnerTypeId());
	 * however, you may wish to override this method in your stub class to provide setting appropriate
	 * to your application -- for example, setting the initial array to the values stored in database.
	 *
	 * @return     void
	 */
	public function initWallIgnoresRelatedByOwnerTypeId()
	{
		$this->collWallIgnoresRelatedByOwnerTypeId = array();
	}
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this PrivacyNodeType has previously been saved, it will retrieve
	 * related WallIgnoresRelatedByOwnerTypeId from storage. If this PrivacyNodeType is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
	 ...
	 * @throws     PropelException
	 */
	public function getWallIgnoresRelatedByOwnerTypeId($criteria = null, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(PrivacyNodeTypePeer::DATABASE_NAME);
		}
		...
		if ($this->collWallIgnoresRelatedByOwnerTypeId === null) {
			if ($this->isNew()) {
			   $this->collWallIgnoresRelatedByOwnerTypeId = array();
				...
				$criteria->add(WallIgnorePeer::OWNER_TYPE_ID, $this->id);

				WallIgnorePeer::addSelectColumns($criteria);
				$this->collWallIgnoresRelatedByOwnerTypeId = WallIgnorePeer::doSelect($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				WallIgnorePeer::addSelectColumns($criteria);
				if (!isset($this->lastWallIgnoreRelatedByOwnerTypeIdCriteria) || !$this->lastWallIgnoreRelatedByOwnerTypeIdCriteria->equals($criteria)) {
					$this->collWallIgnoresRelatedByOwnerTypeId = WallIgnorePeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastWallIgnoreRelatedByOwnerTypeIdCriteria = $criteria;
		return $this->collWallIgnoresRelatedByOwnerTypeId;
	}
	 ...
	 * @throws     PropelException
	 */
	public function countWallIgnoresRelatedByOwnerTypeId(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(PrivacyNodeTypePeer::DATABASE_NAME);
		} else {
		...
		if ($this->collWallIgnoresRelatedByOwnerTypeId === null) {
			...
			if (!$this->isNew()) {
				...
				if (!isset($this->lastWallIgnoreRelatedByOwnerTypeIdCriteria) || !$this->lastWallIgnoreRelatedByOwnerTypeIdCriteria->equals($criteria)) {
					...
					$count = count($this->collWallIgnoresRelatedByOwnerTypeId);
				}
			} else {
				$count = count($this->collWallIgnoresRelatedByOwnerTypeId);
			}
	 ...
	 * @throws     PropelException
	 */
	public function addWallIgnoreRelatedByOwnerTypeId(WallIgnore $l)
	{
		if ($this->collWallIgnoresRelatedByOwnerTypeId === null) {
			$this->initWallIgnoresRelatedByOwnerTypeId();
		}
		if (!in_array($l, $this->collWallIgnoresRelatedByOwnerTypeId, true)) { // only add it if the **same** object is not already associated
			array_push($this->collWallIgnoresRelatedByOwnerTypeId, $l);
			$l->setPrivacyNodeTypeRelatedByOwnerTypeId($this);
		}
	}
	 ...
	 * Otherwise if this PrivacyNodeType is new, it will return
	 * an empty collection; or if this PrivacyNodeType has previously
	 * been saved, it will retrieve related WallIgnoresRelatedByOwnerTypeId from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in PrivacyNodeType.
	 */
	public function getWallIgnoresRelatedByOwnerTypeIdJoinUser($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(PrivacyNodeTypePeer::DATABASE_NAME);
		}
		...
		if ($this->collWallIgnoresRelatedByOwnerTypeId === null) {
			if ($this->isNew()) {
				...
				$this->collWallIgnoresRelatedByOwnerTypeId = WallIgnorePeer::doSelectJoinUser($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	...
	/**
	 * Clears out the collWallIgnoresRelatedByPosterTypeId collection (array).
	 *
	 * This does not modify the database; however, it will remove any associated objects, causing
	 * them to be refetched by subsequent calls to accessor method.
	 *
	 * @return     void
	 * @see        addWallIgnoresRelatedByPosterTypeId()
	 */
	public function clearWallIgnoresRelatedByPosterTypeId()
	{
		$this->collWallIgnoresRelatedByPosterTypeId = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collWallIgnoresRelatedByPosterTypeId collection (array).
	 *
	 * By default this just sets the collWallIgnoresRelatedByPosterTypeId collection to an empty array (like clearcollWallIgnoresRelatedByPosterTypeId());
	 * however, you may wish to override this method in your stub class to provide setting appropriate
	 * to your application -- for example, setting the initial array to the values stored in database.
	 *
	 * @return     void
	 */
	public function initWallIgnoresRelatedByPosterTypeId()
	{
		$this->collWallIgnoresRelatedByPosterTypeId = array();
	}
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this PrivacyNodeType has previously been saved, it will retrieve
	 * related WallIgnoresRelatedByPosterTypeId from storage. If this PrivacyNodeType is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
	 ...
	 * @throws     PropelException
	 */
	public function getWallIgnoresRelatedByPosterTypeId($criteria = null, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(PrivacyNodeTypePeer::DATABASE_NAME);
		}
		...
		if ($this->collWallIgnoresRelatedByPosterTypeId === null) {
			if ($this->isNew()) {
			   $this->collWallIgnoresRelatedByPosterTypeId = array();
				...
				$criteria->add(WallIgnorePeer::POSTER_TYPE_ID, $this->id);

				WallIgnorePeer::addSelectColumns($criteria);
				$this->collWallIgnoresRelatedByPosterTypeId = WallIgnorePeer::doSelect($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				WallIgnorePeer::addSelectColumns($criteria);
				if (!isset($this->lastWallIgnoreRelatedByPosterTypeIdCriteria) || !$this->lastWallIgnoreRelatedByPosterTypeIdCriteria->equals($criteria)) {
					$this->collWallIgnoresRelatedByPosterTypeId = WallIgnorePeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastWallIgnoreRelatedByPosterTypeIdCriteria = $criteria;
		return $this->collWallIgnoresRelatedByPosterTypeId;
	}
	 ...
	 * @throws     PropelException
	 */
	public function countWallIgnoresRelatedByPosterTypeId(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(PrivacyNodeTypePeer::DATABASE_NAME);
		} else {
		...
		if ($this->collWallIgnoresRelatedByPosterTypeId === null) {
			...
			if (!$this->isNew()) {
				...
				if (!isset($this->lastWallIgnoreRelatedByPosterTypeIdCriteria) || !$this->lastWallIgnoreRelatedByPosterTypeIdCriteria->equals($criteria)) {
					...
					$count = count($this->collWallIgnoresRelatedByPosterTypeId);
				}
			} else {
				$count = count($this->collWallIgnoresRelatedByPosterTypeId);
			}
	 ...
	 * @throws     PropelException
	 */
	public function addWallIgnoreRelatedByPosterTypeId(WallIgnore $l)
	{
		if ($this->collWallIgnoresRelatedByPosterTypeId === null) {
			$this->initWallIgnoresRelatedByPosterTypeId();
		}
		if (!in_array($l, $this->collWallIgnoresRelatedByPosterTypeId, true)) { // only add it if the **same** object is not already associated
			array_push($this->collWallIgnoresRelatedByPosterTypeId, $l);
			$l->setPrivacyNodeTypeRelatedByPosterTypeId($this);
		}
	}
	 ...
	 * Otherwise if this PrivacyNodeType is new, it will return
	 * an empty collection; or if this PrivacyNodeType has previously
	 * been saved, it will retrieve related WallIgnoresRelatedByPosterTypeId from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in PrivacyNodeType.
	 */
	public function getWallIgnoresRelatedByPosterTypeIdJoinUser($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(PrivacyNodeTypePeer::DATABASE_NAME);
		}
		...
		if ($this->collWallIgnoresRelatedByPosterTypeId === null) {
			if ($this->isNew()) {
				...
				$this->collWallIgnoresRelatedByPosterTypeId = WallIgnorePeer::doSelectJoinUser($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	...
	/**
	 * Clears out the collWallIgnoresRelatedByItemTypeId collection (array).
	 *
	 * This does not modify the database; however, it will remove any associated objects, causing
	 * them to be refetched by subsequent calls to accessor method.
	 *
	 * @return     void
	 * @see        addWallIgnoresRelatedByItemTypeId()
	 */
	public function clearWallIgnoresRelatedByItemTypeId()
	{
		$this->collWallIgnoresRelatedByItemTypeId = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collWallIgnoresRelatedByItemTypeId collection (array).
	 *
	 * By default this just sets the collWallIgnoresRelatedByItemTypeId collection to an empty array (like clearcollWallIgnoresRelatedByItemTypeId());
	 * however, you may wish to override this method in your stub class to provide setting appropriate
	 * to your application -- for example, setting the initial array to the values stored in database.
	 *
	 * @return     void
	 */
	public function initWallIgnoresRelatedByItemTypeId()
	{
		$this->collWallIgnoresRelatedByItemTypeId = array();
	}
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this PrivacyNodeType has previously been saved, it will retrieve
	 * related WallIgnoresRelatedByItemTypeId from storage. If this PrivacyNodeType is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
	 ...
	 * @throws     PropelException
	 */
	public function getWallIgnoresRelatedByItemTypeId($criteria = null, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(PrivacyNodeTypePeer::DATABASE_NAME);
		}
		...
		if ($this->collWallIgnoresRelatedByItemTypeId === null) {
			if ($this->isNew()) {
			   $this->collWallIgnoresRelatedByItemTypeId = array();
				...
				$criteria->add(WallIgnorePeer::ITEM_TYPE_ID, $this->id);

				WallIgnorePeer::addSelectColumns($criteria);
				$this->collWallIgnoresRelatedByItemTypeId = WallIgnorePeer::doSelect($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				WallIgnorePeer::addSelectColumns($criteria);
				if (!isset($this->lastWallIgnoreRelatedByItemTypeIdCriteria) || !$this->lastWallIgnoreRelatedByItemTypeIdCriteria->equals($criteria)) {
					$this->collWallIgnoresRelatedByItemTypeId = WallIgnorePeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastWallIgnoreRelatedByItemTypeIdCriteria = $criteria;
		return $this->collWallIgnoresRelatedByItemTypeId;
	}
	 ...
	 * @throws     PropelException
	 */
	public function countWallIgnoresRelatedByItemTypeId(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(PrivacyNodeTypePeer::DATABASE_NAME);
		} else {
		...
		if ($this->collWallIgnoresRelatedByItemTypeId === null) {
			...
			if (!$this->isNew()) {
				...
				if (!isset($this->lastWallIgnoreRelatedByItemTypeIdCriteria) || !$this->lastWallIgnoreRelatedByItemTypeIdCriteria->equals($criteria)) {
					...
					$count = count($this->collWallIgnoresRelatedByItemTypeId);
				}
			} else {
				$count = count($this->collWallIgnoresRelatedByItemTypeId);
			}
	 ...
	 * @throws     PropelException
	 */
	public function addWallIgnoreRelatedByItemTypeId(WallIgnore $l)
	{
		if ($this->collWallIgnoresRelatedByItemTypeId === null) {
			$this->initWallIgnoresRelatedByItemTypeId();
		}
		if (!in_array($l, $this->collWallIgnoresRelatedByItemTypeId, true)) { // only add it if the **same** object is not already associated
			array_push($this->collWallIgnoresRelatedByItemTypeId, $l);
			$l->setPrivacyNodeTypeRelatedByItemTypeId($this);
		}
	}
	 ...
	 * Otherwise if this PrivacyNodeType is new, it will return
	 * an empty collection; or if this PrivacyNodeType has previously
	 * been saved, it will retrieve related WallIgnoresRelatedByItemTypeId from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in PrivacyNodeType.
	 */
	public function getWallIgnoresRelatedByItemTypeIdJoinUser($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(PrivacyNodeTypePeer::DATABASE_NAME);
		}
		...
		if ($this->collWallIgnoresRelatedByItemTypeId === null) {
			if ($this->isNew()) {
				...
				$this->collWallIgnoresRelatedByItemTypeId = WallIgnorePeer::doSelectJoinUser($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	...
	/**
	 * Clears out the collPremiumAccounts collection (array).
	 *
	 * This does not modify the database; however, it will remove any associated objects, causing
	 * them to be refetched by subsequent calls to accessor method.
	 *
	 * @return     void
	 * @see        addPremiumAccounts()
	 */
	public function clearPremiumAccounts()
	{
		$this->collPremiumAccounts = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collPremiumAccounts collection (array).
	 *
	 * By default this just sets the collPremiumAccounts collection to an empty array (like clearcollPremiumAccounts());
	 * however, you may wish to override this method in your stub class to provide setting appropriate
	 * to your application -- for example, setting the initial array to the values stored in database.
	 *
	 * @return     void
	 */
	public function initPremiumAccounts()
	{
		$this->collPremiumAccounts = array();
	}

	/**
	 * Gets an array of PremiumAccount objects which contain a foreign key that references this object.
	 *
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this PrivacyNodeType has previously been saved, it will retrieve
	 * related PremiumAccounts from storage. If this PrivacyNodeType is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
	 * @return     array PremiumAccount[]
	 * @throws     PropelException
	 */
	public function getPremiumAccounts($criteria = null, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(PrivacyNodeTypePeer::DATABASE_NAME);
		}
		...
		if ($this->collPremiumAccounts === null) {
			if ($this->isNew()) {
			   $this->collPremiumAccounts = array();
				...
				$criteria->add(PremiumAccountPeer::OWNER_TYPE_ID, $this->id);

				PremiumAccountPeer::addSelectColumns($criteria);
				$this->collPremiumAccounts = PremiumAccountPeer::doSelect($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				PremiumAccountPeer::addSelectColumns($criteria);
				if (!isset($this->lastPremiumAccountCriteria) || !$this->lastPremiumAccountCriteria->equals($criteria)) {
					$this->collPremiumAccounts = PremiumAccountPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastPremiumAccountCriteria = $criteria;
		return $this->collPremiumAccounts;
	}

	/**
	 * Returns the number of related PremiumAccount objects.
	 *
	 * @param      Criteria $criteria
	 * @param      boolean $distinct
	 * @param      PropelPDO $con
	 * @return     int Count of related PremiumAccount objects.
	 * @throws     PropelException
	 */
	public function countPremiumAccounts(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(PrivacyNodeTypePeer::DATABASE_NAME);
		} else {
		...
		if ($this->collPremiumAccounts === null) {
			if ($this->isNew()) {
				...
				$count = PremiumAccountPeer::doCount($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				if (!isset($this->lastPremiumAccountCriteria) || !$this->lastPremiumAccountCriteria->equals($criteria)) {
					...
					$count = count($this->collPremiumAccounts);
				}
			} else {
				$count = count($this->collPremiumAccounts);
			}
	...
	/**
	 * Method called to associate a PremiumAccount object to this object
	 * through the PremiumAccount foreign key attribute.
	 *
	 * @param      PremiumAccount $l PremiumAccount
	 * @return     void
	 * @throws     PropelException
	 */
	public function addPremiumAccount(PremiumAccount $l)
	{
		if ($this->collPremiumAccounts === null) {
			$this->initPremiumAccounts();
		}
		if (!in_array($l, $this->collPremiumAccounts, true)) { // only add it if the **same** object is not already associated
			array_push($this->collPremiumAccounts, $l);
			$l->setPrivacyNodeType($this);
		}
	}
	...
	public function clearAllReferences($deep = false)
	{
		if ($deep) {
			...
			if ($this->collWallIgnoresRelatedByOwnerTypeId) {
				foreach ((array) $this->collWallIgnoresRelatedByOwnerTypeId as $o) {
					$o->clearAllReferences($deep);
				}
			}
			if ($this->collWallIgnoresRelatedByPosterTypeId) {
				foreach ((array) $this->collWallIgnoresRelatedByPosterTypeId as $o) {
					$o->clearAllReferences($deep);
				}
			}
			if ($this->collWallIgnoresRelatedByItemTypeId) {
				foreach ((array) $this->collWallIgnoresRelatedByItemTypeId as $o) {
					$o->clearAllReferences($deep);
				}
			}
			...
			if ($this->collPremiumAccounts) {
				foreach ((array) $this->collPremiumAccounts as $o) {
					$o->clearAllReferences($deep);
				}
			}
		} // if ($deep)
