    ...
    public static function getFeaturedCompanies($maxnum=20, $random = true)
    {
        ...
        $sql = "
            ...
            WHERE EMT_COMPANY.AVAILABLE=1 AND EMT_COMPANY.BLOCKED=0 AND EMT_COMPANY.IS_FEATURED=1
                AND NOT EXISTS (SELECT 1 FROM EMT_BLOCKLIST BLL WHERE BLL.LOGIN_ID=EMT_USER.LOGIN_ID AND BLL.ACTIVE=1)
                AND EXISTS (SELECT 1 FROM EMT_MEDIA_ITEM MI WHERE MI.OWNER_ID=EMT_COMPANY.ID AND MI.OWNER_TYPE_ID=".PrivacyNodeTypePeer::PR_NTYP_COMPANY." AND MI.ITEM_TYPE_ID=".MediaItemPeer::MI_TYP_LOGO.")
                AND EMT_PREMIUM_ACCOUNT.STATUS=".PremiumAccountPeer::PA_STAT_ACTIVE."
