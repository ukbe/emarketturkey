    ...
    public static function getFeaturedProducts($maxnum = 20, $rand = false)
    {
        ...
        $sql = "SELECT * FROM
                (
        			...
        			WHERE EMT_PRODUCT.ACTIVE=1 AND EMT_PRODUCT.DELETED_AT IS NULL AND EMT_COMPANY.AVAILABLE=1 AND EMT_COMPANY.BLOCKED=0 AND EMT_COMPANY.IS_FEATURED=1
                			AND NOT EXISTS (SELECT 1 FROM EMT_BLOCKLIST WHERE EMT_BLOCKLIST.LOGIN_ID=EMT_USER.LOGIN_ID AND ACTIVE=1)
        					AND EXISTS (
